    ...
    def _generate_examples(self, filepath):
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for file_idx, curr_line in enumerate(lines):
                ...
                df = pd.read_csv(df_path).astype(str).sample(frac=1)

                for row_idx, row in df.iterrows():
                    data_id = str(file_idx) + "_" + str(row["doc_id"]) + "_" + str(row_idx)
                    title = row["title"]
                    text = row["text"]
                    src_txt, dst_txt, type_txt = row["src"], row["dst"], row["type"]
                    triplet_txt = f"<triplet> {src_txt} <subj> {dst_txt} <obj> {relation_name_mappings[type_txt]}"
